...
;;;;
;;;; We maintain the list sorted by index descending at insertion time,
;;;; so pop is O(1) and insertion is O(n) in the number of distinct indices.

(defun insert-sorted-descending (entry list)
  "Insert ENTRY into LIST maintaining descending order by (first entry)."
  (let ((index (first entry)))
    (if (or (null list) (> index (first (first list))))
        (cons entry list)
        (cons (first list) (insert-sorted-descending entry (rest list))))))
...
Returns (INDEX . THUNK) or NIL."
  (when (tramp-%negative-listeners tramp)
    ;; List is already sorted descending, so first entry is highest
  ...
  (let* ((tramp (make-trampoline grammar text))
    ...
    (let ((results (run-trampoline tramp)))
      (if results
          ...
          (let ((failure (or (tramp-%failure tramp)
                             (make-failure 0 (list (list :tag :string
                                                         :expecting "complete parse"))))))
            (values (augment-failure failure text) nil))))))

(defun parse-grammar-partial (grammar start-rule text)
  "Parse TEXT using GRAMMAR starting from START-RULE, allowing partial matches.
Returns the first successful parse result (may not consume all input)."
  (let* ((tramp (make-trampoline grammar text))
         (start-parser (gethash start-rule grammar)))
    (unless start-parser
      (error "Start rule ~A not found in grammar" start-rule))

    ;; Register regular listener (not full) for partial parse
    (push-listener tramp
                   (node-key 0 start-parser)
                   (make-top-listener tramp))
